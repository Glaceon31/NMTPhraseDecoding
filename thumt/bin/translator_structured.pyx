...
cdef search_tag_inner(automatons autom, int state_start, int state_end, tags):
    result = ''
    current = state_end
...
cdef search_tag_outer(automatons autom, int state_start, int state_end, tags):
    result = ''
    current = state_start
    while current != state_end:
        if autom.states[current].next_state != -1:
            current = autom.states[current].next_state
...
cdef int find_automatons_state(automatons autom, int srcpos):
    cdef int i, j
    for i in range(autom.state_num):
        for j in range(autom.states[i].num_visible):
            if autom.states[i].visible[j] == srcpos:
                return i
...
cdef print_autom(automatons autom):
    ...
    for i in range(autom.state_num):
        ...
        printf("};")
        if autom.states[i].next_state != -1:
            printf(" next_state %d ;", autom.states[i].next_state)
        if autom.states[i].outer != -1:
            printf(" outer %d ;", autom.states[i].outer)
        if autom.states[i].num_inner > 0:
            printf(" inner {")
            for j in range(autom.states[i].num_inner):
                printf(" %d", autom.states[i].inner[j])
            printf("}; ")
        printf("all visible {")
        for j in range(autom.states[i].num_all_visible):
            printf("%d ", autom.states[i].all_visible[j])
        printf("}; ")
        printf("all visible states{")
        for j in range(autom.states[i].num_all_visible):
            printf("%d ", autom.states[i].all_visible_state[j])
        printf("};")
        printf("\n")



cdef int state_finish(automatons autom, int current_state, int *coverage):
    cdef int i, j, finished
    ...
    for i in range(state.num_inner):
        if state_finish(autom, state.inner[i], coverage) == 0:
            return 0
    return 1
...
cdef int to_next_state(automatons autom, int current_state, int *coverage):
    cdef int i, j, finished
    finished = 1
    state = autom.states[current_state]
    '''
    ...
    for i in range(state.num_inner):
        if finished == 1:
            break
        inner = autom.states[state.inner[i]]
        for j in range(inner.num_visible):
            if coverage[inner.visible[j]] == 0:
                finished = 0
                break
    '''
    ...
    while state_finish(autom, result, coverage):
        ...
        if autom.states[result].next_state != -1:
            ...
            result = autom.states[result].outer
        else:
            return result
        #assert 0 == 1
    return result
...
cdef get_feature_map(translation_status stack[150][4], int *stack_count, translation_status *stack_limit[150], int *stack_limit_count, int len_src, ivocab_trg, hidden_state_pool, params, int maps[150][4], int *maps_limit[150], float threshold, float threshold_limit):
    ...
    for i in range(len(words)):
        ...
        if result.has_key(words[i]):
            try:
                tmpstr = words[i]
                prob = 1.0
                result[words[i]].append([tmpstr, prob])
                #printf("reserved: %s\n", tmpstr)
            except:
...
cdef get_all_visible(automatons autom, int current_state):
    cdef int i, j
    state = autom.states[current_state]
    result_tmp = []
    states = []
    for i in range(state.num_visible):
        result_tmp.append(state.visible[i])
        states.append(current_state)
    for i in range(state.num_inner):
        new_visible, new_states = get_all_visible(autom, state.inner[i])
        result_tmp += new_visible 
        states += new_states
    return result_tmp, states


cdef get_kmax(sorted_array, int *avail, int num, automatons autom, int current_state, golden=None):
    result = []
    newstates = []
    cdef int pos = 0
    cdef int isvisible = 0
    cdef int i, j, newstate
    while len(result) < num and pos < len(sorted_array):
        if avail[sorted_array[pos]] == 0:
            ...
            for i in range(state.num_inner):
                if isvisible:
                    break
                for j in range(autom.states[state.inner[i]].num_visible):
                    if sorted_array[pos] == autom.states[state.inner[i]].visible[j]:
                        isvisible = 1
                        newstate = state.inner[i]
                        break
            '''
            if isvisible:
    ...
    for i in range(stack_count):
        if strcmp(element.translation, stack[i].translation) == 0: 
            if stack[i].loss < element.loss:
                ...
                stack[i].middle_state = element.middle_state
...
cpdef main(args):
    ...
    cdef:
        ...
        translation_status *stacks_limit[150][150]
        int stacks_limit_count[150][150]
        ...
        candidate candidate_phrase_list[150][4]
        candidate *candidate_phrase_list_limit[150]
    ...
    with tf.Graph().as_default():
        ...
        for input in inputs:
                        ...
                        for i in range(stacks_count[length][num_cov]):
                            ...
                            for nosense in range(1):
                                ...
                                if True:
                                    ...
                                    for j in range(autom.states[element.automatons].num_all_visible):
                                        pos = autom.states[element.automatons].all_visible[j]
                                        nextstate = autom.states[element.automatons].all_visible_state[j]
                                        # bpe_phrase
                                        ...
                                        if params_c.bpe_phrase:
                                            ...
                                            if element.coverage[pos_end] == 1:
                                                continue
                                            is_visible = 0
                                            for k in range(autom.states[nextstate].num_visible):
                                                    ...
                                                    for bpe_phrase in plist:
                                                        if phrases.has_key(bpe_phrase):
                                                            ...
                                                            for j in range(len(phrases[bpe_phrase])):
                                                                ...
                                                                new_candidate.next_state = nextstate
                                    ...
                                    for j in range(len_src):
                                        ...
                                        for k in range(candidate_phrase_list_count[j]):
                                            ...
                                            if pos >= 0:
                                                ...
                                                newelement.automatons = nextstate
                                                newelement.middle_state = nextstate
                                                tns = to_next_state(autom, newelement.automatons, newelement.coverage)
                                                if tns != -1:
                                                    newelement.automatons = tns
            ...
            for j in range(autom.state_num):
                if tags[j][0] != '':
                    words_result[autom.states[j].pos_start] = tags[j][0]+' '+words_result[autom.states[j].pos_start]
                if tags[j][1] != '':
                    words_result[autom.states[j].pos_end] += ' '+tags[j][1]
            '''
