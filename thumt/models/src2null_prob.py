...
def decoding_graph(features, state, mode, params):
    ...
    weights = tf.get_variable("weight_prediction", [1, hidden_size],
                              initializer=initializer)
    bias = tf.get_variable("bias_prediction", [1], initializer=tf.zeros_initializer)
    ...
    # label smoothing
    #labels = tf.Print(labels, [tf.shape(labels), tf.shape(logits), tf.shape(decoder_output)], summarize=100 )
...
class Src2null_prob(interface.NMTModel):
    ...
    def get_inference_func(self):
        ...
        def decoding_fn(features, state, params=None):
            ...
            with tf.variable_scope(self._scope, reuse=tf.AUTO_REUSE):
                log_prob = decoding_graph(features, state, "infer",
                                                     params)
